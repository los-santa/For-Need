...
function usePersistedScheduleBudgetState<T>(key: string, defaultValue: T) {
  const [value, setValue] = useState<T>(() =>
    loadScheduleBudgetState(key, defaultValue),
  );
  ...
  useEffect(() => {
    ...
    saveScheduleBudgetState(key, value);
  }, [key, value]);
...
function ScheduleAndBudgetContent() {
  ...
  const [cashAmount, setCashAmount] = usePersistedScheduleBudgetState<number>('cashAmount', 0);
  const [items, setItems] = usePersistedScheduleBudgetState<Item[]>('items', []);
  const [debts, setDebts] = usePersistedScheduleBudgetState<Debt[]>('debts', []);
  const [loans, setLoans] = usePersistedScheduleBudgetState<Loan[]>('loans', []);
  const [cashTransactions, setCashTransactions] = usePersistedScheduleBudgetState<CashTransaction[]>('cashTransactions', []);
  const [budgets, setBudgets] = usePersistedScheduleBudgetState<Budget[]>('budgets', []);
  const [expenses, setExpenses] = usePersistedScheduleBudgetState<Expense[]>('expenses', []);
  const [schedules, setSchedules] = usePersistedScheduleBudgetState<Schedule[]>('schedules', []);
  const [recurringExpenses, setRecurringExpenses] = usePersistedScheduleBudgetState<RecurringExpense[]>('recurringExpenses', []);
