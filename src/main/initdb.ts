...
import { DEFAULT_CARDTYPE_BACKFILL_SQL } from './cardtypeMigration';
...
// 기존 카드 중 타입이 없는 카드만 'todo' 카드타입으로 마이그레이션
try {
  ...
  if (todoCardType) {
    const updateResult = db.prepare(DEFAULT_CARDTYPE_BACKFILL_SQL)
