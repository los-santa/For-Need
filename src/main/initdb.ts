...
// 기존 카드 중 타입이 비어 있는 항목만 'todo' 카드타입으로 마이그레이션
try {
  console.log('Migrating untyped cards to todo cardtype...');
  ...
  if (todoCardType) {
    ...
    if (updateResult.changes > 0) {
      console.log(`Updated ${updateResult.changes} untyped cards to 'todo' cardtype`);
    }
  }
} catch (error) {
