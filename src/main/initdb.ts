...
// 기존 무타입 카드들을 현재 기본 카드타입으로 마이그레이션
try {
  console.log("Migrating unset cards to 'no type yet' cardtype...");

  const updatedCardCount = migrateUnsetCardTypesToNoTypeYet(db);

  if (updatedCardCount > 0) {
    console.log(`Updated ${updatedCardCount} unset cards to 'no type yet' cardtype`);
  }
} catch (error) {
