...
import { migrateCardsWithoutTypeToTodo } from './cardtypeMigration';
...
// 카드타입 컬럼이 추가되기 전의 기존 카드만 'todo' 카드타입으로 마이그레이션
try {
  console.log('Migrating cards without cardtype to todo cardtype...');

  const updatedCardCount = migrateCardsWithoutTypeToTodo(db);

  if (updatedCardCount > 0) {
    console.log(`Updated ${updatedCardCount} cards to 'todo' cardtype`);
  }
} catch (error) {
