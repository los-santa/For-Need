...
import { migrateCardsMissingTypeToTodoSql } from './dbSafety';
...
// 타입 정보가 없는 기존 카드만 'todo' 카드타입으로 마이그레이션
try {
  console.log('Migrating cards without a valid cardtype to todo cardtype...');

  // 'todo' 카드타입 ID 가져오기
  const todoCardType = db.prepare("SELECT cardtype_id FROM CARDTYPES WHERE cardtype_name = 'todo'").get() as any;

  if (todoCardType) {
    const updateResult = db.prepare(migrateCardsMissingTypeToTodoSql)
      .run(todoCardType.cardtype_id);

    if (updateResult.changes > 0) {
      console.log(`Updated ${updateResult.changes} cards without a valid cardtype to 'todo' cardtype`);
    }
  }
} catch (error) {
