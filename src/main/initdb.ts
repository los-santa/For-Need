...
import { migrateMissingCardtypesToTodo } from './databaseMigrations';
...
// 카드타입이 없던 레거시 카드만 'todo' 카드타입으로 마이그레이션
try {
  console.log('Migrating cards with missing cardtype to todo cardtype...');

  const changedCards = migrateMissingCardtypesToTodo(db);

  if (changedCards > 0) {
    console.log(`Updated ${changedCards} cards with missing cardtype to 'todo'`);
  }
} catch (error) {
