...
// 기존 NULL 카드타입만 'todo' 카드타입으로 보정
try {
  console.log('Migrating cards with missing cardtype to todo...');

  const updatedCards = migrateMissingCardTypesToTodo(db);

  if (updatedCards > 0) {
    console.log(`Updated ${updatedCards} cards with missing cardtype to 'todo'`);
  }
} catch (error) {
