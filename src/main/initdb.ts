...
import { migrateNullCardTypesToTodo } from './cardDb';
...
// 기존 NULL 카드타입만 'todo'로 보정한다. 이미 지정된 타입은 사용자 데이터이므로 보존한다.
try {
  console.log('Migrating cards with NULL cardtype to todo cardtype...');

  const migratedCount = migrateNullCardTypesToTodo(db);

  if (migratedCount > 0) {
    console.log(`Updated ${migratedCount} cards with NULL cardtype to 'todo'`);
  }
} catch (error) {
